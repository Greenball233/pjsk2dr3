...
def process_score(lines: list[tuple[str]], metadata: list[tuple[str]]) -> Score:
    ...
    try:
        ticks_per_beat_request = [int(request.split()[1]) for request in processed_metadata.requests if
                                  request.startswith('ticks_per_beat')] if processed_metadata.requests else []
    ...
    bars = list(reversed(
        [
            (
                ...
                ticks := ticks +
                         int((measure - sorted_bar_lengths[i - 1][0]) * sorted_bar_lengths[i - 1][
                             1] * ticks_per_beat if i > 0 else 0)
